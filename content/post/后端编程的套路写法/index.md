...
**首先**先要肯定，这两个 **变量地址**初始化都是ok的    
但是没有赋值、或者是空指针的**地址变量**，不能进行大部分的**访问对象的成员**的**行为**           

**没有赋值**的情况下，在出现 **访问对象的成员** 的行为时，会**编译时报错**   
而**赋值空指针**的情况下，是**运行时报错**    

* 我们初始化**待定的变量**的时候要**避免没有赋值地址**的情况，   
更推荐使用**null赋值**，   
因为null值的**空指针报错**可以用`if(array != null)`进行规避，更灵活   
而且**空指针**，也是指针，是可以打印的，就是可以使用`array.toString()`

***

**一个完整的工程**   
我们在一个**包**下创建的多个类，实现一个程序   
* **有几个必要出现的类：**    
```java
    // 框架类
    public class Frame{
    ...
    public class FrameController{
        public static void main(String[] args) {
// 整个程序唯一的启动类，放着整个程序唯一的main程序入口方法
            new Frame();
            //执行结构框架 
        }
    }
...
在创建类、创建模板的时候要记住：    
    ...
    * **static的属于【类】，改动了就是改【模板】**

    * **static{}与【类】同时出生，public 类名(){}与【对象】同时出生**
