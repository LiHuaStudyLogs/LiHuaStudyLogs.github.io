...
+++
引言： 在操作系统上安装JDK是为了让我们写的java程序在我们的电脑上跑起来

## 核心概念
### 什么是JDK？
* **JDK，是java后端语言的开发工具包**  
包含两个组成部分（java.c --- java --- JRE）：
    > 1. java.c<sup>编译工具</sup> --> 实现把 **源文件** (.java)变成 **类文件** (.class),  
    **这个过程就是让编程语言能被计算机<sup>JVM,即java虚拟机</sup> 读懂的语言** |   
    JVM虚拟机 解析 **类文件** ，编译成 **操作系统** 能读懂的指令，  
    java<sup>执行工具</sup> --> 执行指令

    > 2. JRE,是java的运行环境  
        包含两个组成部分（JVM虚拟机 --- java提供的方法）:  
        >>  JVM虚拟机  
        >>  java提供的方法：  
...
### 为什么配置环境变量？
* 告诉 **操作系统** 可执行程序在哪里 --> 所以 **环境变量** 指向JDK的`bin目录`<sup>存放java、javac</sup>  
**配置全局环境变量 --> 电脑任意目录下能执行java相关命令**



## 安装操作<sup>windows系统</sup>
1. 拿到JDK的安装包  
2. 配置全局环境变量：
