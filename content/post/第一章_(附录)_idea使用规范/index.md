...
+++
引言：作为附录，在上面的一章中，JDK安装完成之后，我们已经能自由的敲代码并让代码跑起来了，但是idea开发环境，会让我们更轻松的敲代码
...
5. 自动导包配置:使用类的方法时，自动导入对应的包。免去手动操作
...
`选中目标+ctrl + alt + t` --> 快速构建【try-catch】结构

*** 
## java提供的API

### **java.lang包**
> 存放基础类,不用import 导入,JVM会自己加载  --> 聚焦对象模型、字符串处理、线程管理的功能
* **Math类**
```java
    功能，实现简单的数学需求
    -----------------
...
### **java.util包**
> 存放的类需要手动导入  --> 聚焦数组操作、输入处理、集合存储的功能
* **Scanner类**  
```java
    功能：扫描输入流中的数据
    -----------------
