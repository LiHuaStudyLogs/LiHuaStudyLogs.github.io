...
这种**编译时**的异常，是**强制**你对**可能出现**该异常的语句**做处理（捕获/抛出）**或者**在方法声明throws**告诉他你知道这里很容易出错，不用为我担心      
    * 对编译时异常， 采用**在方法声明throws**，是有快捷键的`选中编译时异常 + alt + enter(也就是回车)` 

    * 编译时异常是**代码局部**出现红色，如果是**全红**的高亮，那他妈是明显的语法错误
...
// 一旦选择用抛出异常，这个一整个【方法体】就被终止了，【执行语句4】不能再执行了 
            ...
            执行语句 ；
//  即使在try结构中【抛出异常】了，【finally】还是会执行    
//  即使在try结构中【return】了，【finally】也会执行
        }
